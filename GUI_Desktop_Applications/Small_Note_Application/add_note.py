...
class AddNote(QDialog):
    ...
    def _setup_ui_add_new_note(self):
        label_title = QLabel("Add a new Note")
        ...
        btn_exit_new_note_window = QPushButton("Exit")
        btn_exit_new_note_window.setFont(QFont("Roboto Mono", 12))
        btn_exit_new_note_window.setMinimumSize(0, 30)
        btn_exit_new_note_window.clicked.connect(self.confirm_exit)
        ...
        layout_btn.addWidget(btn_exit_new_note_window)
    ...
    def save_note(self):
        ...
        self.signal_note.emit(notes_title, note)
        self.close()
